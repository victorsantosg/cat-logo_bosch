...
class ECUManagerApp:
    ...
    def _build_ui(self):
        ...
        # --- Grid ---
        self.tree_frame = ctk.CTkFrame(self.root, fg_color="#2b2b2b", corner_radius=15)
        self.tree_frame.pack(fill="both", expand=True, padx=pad, pady=pad)
        ...
        for col, w in zip(columns, [300, 400, 250]):
        ...
        crud_frame = ctk.CTkFrame(self.root, fg_color="#2b2b2b", corner_radius=15)
        crud_frame.pack(fill="x", padx=pad, pady=(0, pad))
        ctk.CTkLabel(crud_frame, text="Número BOSCH:").grid(row=0, column=0, padx=6, pady=6, sticky="w")
    ...
    def on_search(self):
        self.refresh_grid(self.search_bosch.get().strip(),
                          ...
                          self.search_fabricante.get().strip())
        self.set_status("Busca atualizada")
    ...
    def on_tree_select(self, event):
        ...
        if sel:
            ...
            self.entry_bosch.delete(0, "end"); self.entry_bosch.insert(0, vals[0])
            self.entry_model.delete(0, "end"); self.entry_model.insert(0, vals[1])
            self.entry_fabricante.delete(0, "end"); self.entry_fabricante.insert(0, vals[2])
    ...
    def on_save_edit_clicked(self):
        ...
        if not num or not model or not fabri:
            messagebox.showwarning("Aviso", "Preencha todos os campos.")
            return
        if update_ecu(self.selected_id, num, model, fabri):
            self.refresh_grid()
        else:
            messagebox.showerror("Erro", "Falha ao atualizar registro.")
        ...
        try:
            with open(path, newline="", encoding="utf-8") as f:
                ...
                for row in reader:
                    ...
                    fabri = row.get("fabricante") or row.get("Fabricante")
                    if num and model and fabri:
...
def main():
    ...
    ctk.set_default_color_theme("blue")
    root = ctk.CTk()
    app = ECUManagerApp(root, nome_usuario)
    root.mainloop()
